...
import com.restaurantclient.databinding.ItemCashierOrderBinding
import com.restaurantclient.ui.common.setupGlassEffect
import com.restaurantclient.util.DateTimeUtils

/**
 * Adapter for Cashier Order List.
 * Uses ItemCashierOrderBinding (reusing layout) but logic is specific to Cashier workflow.
 */
class CashierOrderAdapter(
    private val onStatusChange: (OrderResponse, String) -> Unit
) : ListAdapter<CashierOrderUIModel, CashierOrderAdapter.OrderViewHolder>(DiffCallback) {
    ...
    inner class OrderViewHolder(
        private val binding: ItemCashierOrderBinding
        ...
        fun bind(uiModel: CashierOrderUIModel) {
            val order = uiModel.order
            binding.orderIdText.text = "Order #${order.order_id}"
            
            val amount = order.total_amount
            binding.orderAmountText.text = java.text.NumberFormat.getCurrencyInstance().format(amount ?: 0.0)
        ...
        private fun buildMetaText(uiModel: CashierOrderUIModel): String {
            val date = DateTimeUtils.formatIsoDate(uiModel.order.created_at)
            return "${uiModel.username} • Placed $date"
        }
    }

    private object DiffCallback : DiffUtil.ItemCallback<CashierOrderUIModel>() {
        override fun areItemsTheSame(oldItem: CashierOrderUIModel, newItem: CashierOrderUIModel): Boolean =
            oldItem.order.order_id == newItem.order.order_id

        override fun areContentsTheSame(oldItem: CashierOrderUIModel, newItem: CashierOrderUIModel): Boolean =
            oldItem == newItem
    }

    override fun submitList(list: List<CashierOrderUIModel>?) {
